...
def save_shards(anndata_slices, output_prefix):
    ...
    for shard_index in range(number_of_shards):
        ...
        shard.write(f"{output_prefix}.h5ad")
        del shard
    del combined
    return total_cells

def collect_cells(required_cells, description):
    included_phenotypes = ['disease', 'tissue_general', 'adult', 'cell_type']
    anndata_slices = []
    total_cells = 0
...
required_cells = {("normal",): CELLS_PER_TYPE for disease in frequent_diseases}
required_cells = {("normal", "lung", "yes", "type II pneumocyte"): CELLS_PER_TYPE,
                    ("lung adenocarcinoma", "lung", "yes", "malignant cell"): CELLS_PER_TYPE,
                   }
anndata_slices = collect_cells(required_cells, f"Collecting representation test set")
saved_cells = save_shards(anndata_slices, OUTPUT_DIR + "lung_cells")

...
if __name__ == "__main__":
    ...
    for disease in [f.split('_')[0] for f in os.listdir(EMBEDDINGS_DIR) if "_embedding" in f]:
        ...
        saved_embeddings = pd.read_pickle(EMBEDDINGS_DIR + disease + '_embeddings.pkl')
        saved_cells = sc.read_h5ad(EMBEDDINGS_DIR + disease + "_cells.h5ad")
        if not disease in ["small cell lung carcinoma", "large lung cell carcinoma"]: continue
        ...
        for healthy_embedding, disease_embedding in tqdm(zip(healthy_cells, disease_cells), desc='geodesics'):
            ...
            ig_paths.append( gamma.path_integrated_gradients(saved_cells, discrete_mask, mod, tok, phenotype_value=disease, disease_ontology_id = disease_ontology_id) ) 
    ...
    pd.to_pickle(results, EMBEDDINGS_DIR + f"geodesics.pkl")
    #import pandas as pd, numpy as np
